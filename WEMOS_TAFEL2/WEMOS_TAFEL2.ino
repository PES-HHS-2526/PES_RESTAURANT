/*!
 * @file WEMOS_TAFEL2.ino
 * @brief Hoofdprogramma voor de Wemos D1 Mini van tafel 2 binnen het PES restaurantproject.
 *
 * Dit bestand bevat de hoofdlogica voor de klantoproepmodule van tafel 2.
 * Wanneer een klant op de knop drukt, gaat de LED bij de tafel aan en wordt
 * via WiFi/TCP het bericht "tafel2:1" naar Raspberry Pi-A gestuurd.
 *
 ...
 * zodat zichtbaar wordt dat tafel 2 hulp nodig heeft of een bestelling klaarstaat.
 *
 * Wanneer de serveerster in de keuken op de resetknop drukt, wordt via de keten
 * STM32 -> Raspberry Pi-B -> Raspberry Pi-A -> Wemos het bericht "tafel2:0"
...
/*!
 * @brief GPIO-pin waarop de knop van tafel 2 is aangesloten.
 *
...
/*!
 * @brief GPIO-pin waarop de LED van tafel 2 is aangesloten.
 *
 ...
 *
 * Deze variabele voorkomt dat het bericht "tafel2:1" herhaaldelijk wordt
...
/*!
 * @brief Hoofdlus voor de klantoproep van tafel 2.
 *
 ...
 * de tafel-LED aangezet en wordt het bericht "tafel2:1" naar Raspberry Pi-A
 * gestuurd.
 *
 * Wanneer het bericht "tafel2:0" wordt ontvangen, betekent dit dat de oproep
...
if (incoming != "") {
    ...
    if (button.isPressed() && !ledState) {
        ledState = true;
        wifi.sendMessage("tafel2:1");
        digitalWrite(ledPin, HIGH);
    }

    if (incoming == "tafel2:0") {
        ledState = false;
        wifi.sendMessage("tafel2:0");
        digitalWrite(ledPin, LOW);
    }
}